...
public:
    ...
    template<typename T> T get(int index) const { return count_getter<T(MtObject*, int)>()(m_owner, index); }
    template<typename T> void set(int index, T value) const { count_setter<void(MtObject*, int, T)>()(m_owner, index, value); }
